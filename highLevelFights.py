...
def plot_player_positions_by_place(
    parser: DemoParser,
    ticks: List[int] = None,
    ax=None,
    save_path: str = None,
    ...
    try:
        if ticks:
            df = parser.parse_ticks(
                ["last_place_name", "X", "Y", "player_name"], ticks=ticks
            )
        else:
    ...
    if place_col is None or x_col is None or y_col is None:
        raise RuntimeError(
            "Required columns ('last_place_name','X','Y') not found in parsed tick data"
        )
    ...
    if annotate and player_col in df.columns:
        for _, r in df.iterrows():
            ax.text(
                r[x_col], r[y_col], str(r.get(player_col, "")), fontsize=6, alpha=0.9
            )
...
def plot_activity_with_decay(
    ...
    if show:
        plt.show()
def plot_round_objectives(
    plottingDataPerRound: List[Any],
    zones: List[str],
    round_number: int,
    tick_rate: float = 64.0,
    show: bool = True,
    ...
    Supports two input shapes for plottingDataPerRound:
      - a list of plottingData dicts (one per zone) matched by position to `zones`
      - a list of tuples (zone_name, objectives_list, plottingData)

    tick_rate: used when calling plot_activity_with_decay.
    show: if True call plt.show() at end.
    """
    import matplotlib.pyplot as plt
    import math

    # ensure at least 7 slots (user note)
    n_slots = max(7, len(zones))
    # choose a reasonable grid
    ncols = 4
    nrows = math.ceil(n_slots / ncols)

    fig, axes = plt.subplots(
        nrows=nrows, ncols=ncols, figsize=(4 * ncols, 3 * nrows), sharex=False
    )
    # flatten axes to a list
    ...
    # Build mapping zone_key -> (objectives, plottingData, original_zone_name)
    entries_by_zone = {}
    if not plottingDataPerRound:
        plottingDataPerRound = []

    # detect element format
    first = plottingDataPerRound[0] if plottingDataPerRound else None
    if first is None:
        entries_by_zone = {}
    elif isinstance(first, dict):
        # treat as ordered list aligned with zones
        for i, pdict in enumerate(plottingDataPerRound):
            if i < len(zones):
                key = _norm(zones[i])
                entries_by_zone[key] = (None, pdict, zones[i])
    else:
        # assume iterable of tuples (zone, objectives, plottingData)
        for elem in plottingDataPerRound:
            try:
                zone_name, objectives, plotData = elem
            except Exception:
                continue
            entries_by_zone[_norm(zone_name)] = (objectives, plotData, zone_name)

    # fill slots with provided zones (or slot labels)
    for slot_idx in range(n_slots):
        ax = axes_list[slot_idx]
        ...
        if key in entries_by_zone:
            ...
            pd_copy = dict(plotData) if isinstance(plotData, dict) else {}
            pd_copy.setdefault("round_number", round_number)
            pd_copy.setdefault("zone_name", zone)
            try:
                plot_activity_with_decay(
                    pd_copy, tick_rate=tick_rate, ax=ax, show=False, show_legend=False
                )
            except Exception:
                ax.text(
                    0.5,
    ...
    # hide any extra axes (if grid larger than n_slots)
    for extra_ax in axes_list[n_slots:]:
        extra_ax.set_visible(False)

    # Collect legend entries from all axes, deduplicate and sort alphabetically
    handle_map = {}
    for ax in axes_list[:n_slots]:
        h, l = ax.get_legend_handles_labels()
        for hh, ll in zip(h, l):
            if ll not in handle_map:
                handle_map[ll] = hh

    if handle_map:
        sorted_labels = sorted(handle_map.keys(), key=lambda s: str(s).lower())
        sorted_handles = [handle_map[l] for l in sorted_labels]
        fig.legend(
            sorted_handles,
            sorted_labels,
            loc="upper center",
            ncol=min(8, len(sorted_labels)),
            bbox_to_anchor=(0.5, 0.99),
        )

    title = (
        f"Round {round_number} Objectives"
        if round_number is not None
        else "Round Objectives"
    )
    fig.suptitle(title)
...
def getRoundWinnerDict(parser: DemoParser) -> Dict[int, str]:
    df = parser.parse_event(
        "round_freeze_end", other=["team_rounds_total", "total_rounds_played"]
    )
    winReason = parser.parse_event("round_end")
    winReason = winReason[winReason["reason"].notna()]
    winReason = winReason.reset_index(drop=True)  # reset index
    winners: Dict[int, str] = {}

    print(len(winReason))
    print(len(df))

    for idx, row in winReason.iterrows():
        winners[idx] = row["reason"]

    return winners
class WorkflowLog:
    def __init__(self):
        # Misc
        self.folder_path = r"./demo_files"
        try:
        ...
        self.HALF_LIFE_ACTIVTIY_POINTS = (
            3  # Im assuming 8 seconds skirmishes... is that right?
        )
        ...
        self.POINT_FOR_KILL = 100
        self.DAMAGE_PER_POINT_RATIO = 0.5
        self.EVEN_COEFFICENT = (
            1.3  # if points is even by 30%, then  #TODO fix when 0 dmg 0 kills
        )
        ...
        self.FILTER_FRACTION_EVENTS_AFTER_MAPPING_PER_ROUND = (
            0.90  # If filtered events are more than 10%, dont consider this level
        )
        ...
        self.ROUND = "ROUND"
        self.FRACTION_ACTIVITY_POINTS_BEFORE_AFTER = (
            "FRACTION_ACTIVITY_POINTS_BEFORE_AFTER"
        )
    ...
    def plotAllCashData(self):
        ...
        # Show quantile lines
        plt.axhline(
            q25, color="green", linestyle="--", label=f"25th percentile: {q25:.0f}"
        )
        plt.axhline(
            q50,
            color="orange",
            linestyle="--",
            label=f"50th percentile (median): {q50:.0f}",
        )
        plt.axhline(
            q75, color="red", linestyle="--", label=f"75th percentile: {q75:.0f}"
        )
    ...
    def getRoundCashSpentEvenDict(self, parser: DemoParser) -> Dict[int, bool]:
        ...
        ticks = list(df["tick"])
        dfticks = parser.parse_ticks(
            ["team_name", "player_name", "cash_spent_this_round"], ticks=ticks
        )
    ...
    def mapPlacesToZones(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not validKeys:
            print(
                "[mapPlacesToZones] No place keys found in DataFrame. Columns:",
                list(roundEvents.columns),
            )
            roundEvents["zone"] = "NO_ZONE"
            return roundEvents[roundEvents["zone"] != "NO_ZONE"]

        best_key = None
        best_activity_sum = -1

        for key in validKeys:
            mapped = roundEvents[key].map(self.ZONE_MAP)
            mask_valid = mapped.notna()

            # Identify unmapped rows
            unmapped_rows = roundEvents.loc[~mask_valid, key]

            # Delete rows where the value is the empty string
            empty_string_mask = unmapped_rows == ""
            if empty_string_mask.any():
                roundEvents = roundEvents.drop(unmapped_rows[empty_string_mask].index)

                # Recompute mapping after deletion
                mapped = roundEvents[key].map(self.ZONE_MAP)
                mask_valid = mapped.notna()

            # After deleting empty strings, check if any unmapped values remain
            remaining_unmapped = roundEvents.loc[~mask_valid, key].dropna().unique()
            if len(remaining_unmapped) > 0:
                print(list(remaining_unmapped))
                raise ValueError("Found unmapped non-empty values.")

            # Now safe to compute activity sum
            ...
            if activity_sum > best_activity_sum:
                ...
                if best_key is None:
                    print(
                        "[mapPlacesToZones] No best_key selected. Returning empty mapping."
                    )
                    roundEvents["zone"] = "NO_ZONE"
                    return roundEvents[roundEvents["zone"] != "NO_ZONE"]

        # Assign zones using the key that maximizes total activityPoints
        # keep a copy to report unmapped originals
        chosen_values = roundEvents[best_key]
        mapped_chosen = chosen_values.map(self.ZONE_MAP)
        unmapped_after_choice = chosen_values[mapped_chosen.isna()].dropna().unique()

        roundEvents["zone"] = mapped_chosen.fillna("NO_ZONE")
        kept = roundEvents[roundEvents["zone"] != "NO_ZONE"]
        return kept
    ...
    def prepareEvents(self, matchNumber) -> pd.DataFrame:
        ...
        for gameEvent in gameEvents:
            ...
            if gameEvent not in list1:
                print(f"{gameEvent}: game event missing!")
                continue

            dfEvents = self.parser.parse_event(
                gameEvent,
                player=[
                    "team_name",
                    "last_place_name",
                ...
                other=["team_score_overtime"],
            )
            dfEvents["event"] = gameEvent
            if gameEvent == "weapon_fire":
                ...
                tick_threshold = int(
                    self.TICK_RATE * self.DELAY_WEAPON_FIRE_RECORDING_SPRAY
                )
                ...
                for idx, row in dfEvents.iterrows():
                    ...
                    if (
                        not last_ticks_dict[key]
                        or tick - last_ticks_dict[key][-1] > tick_threshold
                    ):
                ...
                sprayFilterData = {
                    self.DIFF_AFTER_SPRAY_FILTER: eventsBeforeSprayFilter
                    - eventsAfterSprayFilter,
                ...
                dfEvents["activityPoints"] = np.clip(
                    maxPoints * (damage / 100), a_min=minPoints, a_max=None
                )
            ...
            if not recording:
                if activityPoints >= upper_threshold:
                    ...
                    while True:
                        ...
                        prev_mask = (df_zone["tick"] >= window_start) & (
                            df_zone["tick"] <= spike_tick
                        )
                        ...
                        if overlapping_seg_idx is not None:
                            ...
                            union_idxs = s_idx_set.union(prev_row_indices).union(
                                included_indices
                            )
                            merged_df = (
                                df_zone.loc[sorted(union_idxs)]
                                .sort_values(["tick", "orig_index"])
                                .reset_index(drop=True)
                            )
                            high_activity_segments[overlapping_seg_idx] = merged_df
                            high_activity_index_sets[overlapping_seg_idx] = (
                                set(df_zone.loc[merged_df.index].index.tolist())
                        ...
                        prev_list = [
                            prow.to_dict()
                            for _, prow in prev_rows.sort_values(
                                ["tick", "orig_index"]
                            ).iterrows()
                        ]
                ...
                if activityPoints >= lower_threshold:
                    ...
                    if current_segment_rows:
                        high_activity_segments.append(
                            pd.DataFrame(current_segment_rows)
                        )
    ...
    def determineActivties(self, objective: pd.DataFrame):
        ...
        for _, row in damagedf.iterrows():
            damage = (
                np.minimum(row["damage_capped"], row["user_health"])
                * self.DAMAGE_PER_POINT_RATIO
            )
            if row["attacker_team_name"] == "TERRORIST":
                tDamage += damage
            else:
        ...
        # Impact thresholds
        ctHasImpact = (ctKills > 0) or (ctDamage >= 75)
        tHasImpact = (tKills > 0) or (tDamage >= 75)

        # If neither team had impact → always Even
        if not ctHasImpact and not tHasImpact:
            isEven = True

        # If only one team had impact → that team wins
        elif ctHasImpact and not tHasImpact:
            isEven = False
            ctWin = True

        elif tHasImpact and not ctHasImpact:
            isEven = False
            ctWin = False

        # If both had impact → use ratio logic
        else:
            bigger = max(ctPoints, tPoints)
            smaller = min(ctPoints, tPoints)
            ratio = smaller / bigger if bigger > 0 else 1.0
            isEven = ratio >= self.EVEN_COEFFICENT
            ctWin = ctPoints > tPoints

        # produce final activity label
        if isEven:
    ...
    def _createLog(self, traces):
        ...
        for (
            ...
            trace_dict = {
                ...
                "cashDiff": diff,
                "mapEventAccuracy": round(
                    (
                        self.allFilterDataPerRound[roundNum][
                            "FRACTION_ACTIVITY_POINTS_BEFORE_AFTER"
                        ]
                    ),
                    2,
                ),
                "winReason": roundWinnerDict[roundNum],
        ...
        for matchNumber, match_file in enumerate(self.matches):
            ...
            for round in range(len(roundWinnerDict)):
                ...
                for zone in self.zones:
                    objectives, plottingData = self.findHighActivitySegmentsInZone(
                        zone, roundEvents
                    )
                    if self.PLOTTING:
                        pltDatas.append(plottingData)
                        # plot_activity_with_decay(plottingData, tick_rate=self.TICK_RATE, show_legend=True)
                    for objective in objectives:
                        activity_dict = self.determineActivties(objective)
                        if activity_dict:
                            trace.append(activity_dict)

                sortedTraces = sorted(trace, key=lambda x: x["finalTick"])
                traces.append(
                    (
                        matchNumber,
                        round,
                ...
                if self.PLOTTING:
                    print(round, matchNumber)
                    plot_round_objectives(pltDatas, self.zones, round)
...
def convert_to_event_log(log_dict):
    ...
    for case_id, case_data in log_dict.items():
        ...
        for activity in case_data.get("trace", []):
            ...
            event_name = (
                activity.get("activtiy")
                or activity.get("activity")
                or activity.get("concept:name")
                or "UnknownActivity"
            )
...
if __name__ == "__main__":
    ...
    pm4py.write_xes(event_log, "./logs/high_Level_Fights.xes")
    write_txt_log(log, "LogN.txt")
